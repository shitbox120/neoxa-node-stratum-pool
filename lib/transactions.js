...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData, devSubsidy){
    ...
    if(rpcData.donation_amount && rpcData.donation_payee){
        var devSubsidy = util.addressToScript(rpcData.donation_payee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(rpcData.donation_amount),
            util.varIntBuffer(devSubsidy.length),
            devSubsidy
        ]));
    }
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, coinbase, devSubsidy){
    ...
    var outputTransactions = generateOutputTransactions(publicKey, recipients, rpcData, devSubsidy);
